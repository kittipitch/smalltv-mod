...
> - **Optional daemon source-IP filter** — restrict data pushes (usage/
>   calendar/weather/quota pages) to one IP, so a daemon pointed at the
>   wrong device can't overwrite the screen (not a security feature — this
>   is plaintext HTTP, LAN-only)
...
> calendar/weather/daemon-IP setup).
...
> then `smalltv-mod-firmware-sdpro.bin` (708,224 B, fits the loader's own
> ~2.8 MB OTA slot) to the loader's `POST /update` field `firmware`. Between
...
- **Claude usage meter.** An animated pixel mascot plus your 5-hour and 7-day usage as big percentages with fill bars and reset countdowns. It is fed over WiFi by the [clawdmeter-daemon](https://github.com/kittipitch/clawdmeter-daemon) on your PC. The mascot animates while the device is waiting for its first data after a boot; once data has arrived and then goes stale, the page says **DAEMON OFFLINE** with the age of the last reading instead, so a dead feed can't be mistaken for a live one. Running several devices, the daemon discovers them over mDNS and pushes to all of them.
...
- **Agenda + Weather/AQI.** Your next few Google Calendar events (up to 6, split across two rotating pages if you have that many), color-coded by source calendar, multi-day events shown as a compact date range instead of looking like a same-day event. A separate page shows current temperature, rain %, condition, city name, and a 6-band color-coded AQI + PM2.5 reading, plus a 3-day forecast page. Calendar comes only from the daemon. Weather and AQI are pushed by the daemon too, but the device also has its own direct Open-Meteo fallback: if the daemon goes quiet it fetches the forecast and air quality itself over plain HTTP, so a device with no daemon still shows weather. That fallback doubles as the device's reachability probe (see the offline notices below).
- **It tells you when it is offline, and which side broke.** The device
  classifies its own link and takes over the screen after 30 s rather than
  quietly showing stale numbers. **DEVICE OFFLINE** (red) means this device
  cannot reach the network — `no wifi` when the association is gone, or
  `no internet` when it is associated but two independent hosts both failed to
  answer, in which case it also prints the IP it is associated at so you know
  where to look. **DAEMON OFFLINE** (yellow) means the opposite: the device is
  fine and every other page still has its data, only the feed stopped. Both
  notices show how long it has been that way. An HTTP error from a server —
  a 429, a 500, an unparseable body — counts as proof the link is UP, so a
  flaky API never fakes an outage.
...
curl -m 180 -H 'Expect:' -F "update=@smalltv-mod-loader.bin" http://$IP/update_ota
# screen goes dark, device leaves your LAN -- expected, this is the loader
# booting (it has no display code and no saved WiFi). Join the open
# SmallTV-Loader access point it now opens, then confirm it's alive there:
curl -o /dev/null -w '%{http_code}\n' http://192.168.4.1/update   # want 200
curl -o /dev/null -w '%{http_code}\n' http://192.168.4.1/         # want 404

# 2) real firmware, into the LOADER's updater -- field name is "firmware"
curl -m 300 -H 'Expect:' -F "firmware=@smalltv-mod-firmware-sdpro.bin" http://192.168.4.1/update
...
3. Open **WiFi**, scan, pick your 2.4 GHz network, enter the password, and save. The device reboots and joins.
   If the page stays on **Loading...** and the tabs won't respond, try a phone, or save the network from a terminal while still on `SmallTV-Setup` (details and caveats in the [setup guide](https://kittipitch.github.io/smalltv-mod/getting-started/setup/#if-the-setup-page-wont-finish-loading)):
   `curl -s -m 20 -H 'Content-Type: application/json' -d '{"wifi":[{"ssid":"YOUR_SSID","pass":"YOUR_PASSWORD"}]}' http://192.168.4.1/api/config`
...
- **Setup guides for the parts people get stuck on:**
  [Google Calendar (service account)](https://kittipitch.github.io/smalltv-mod/getting-started/google-calendar/) — click by
  click, with the calendar-share step everyone misses;
  [what the daemon needs installed](https://kittipitch.github.io/smalltv-mod/getting-started/daemon-requirements/) — per
  feature, including `translate-shell` for non-English event titles;
  [sharing your laptop's WiFi](https://kittipitch.github.io/smalltv-mod/getting-started/sharing-wifi/) — when you need a
  2.4 GHz hotspot and how to make one on macOS, Ubuntu or Windows
- [Keep the daemon running](https://kittipitch.github.io/smalltv-mod/getting-started/keep-it-running/) — autostart on Windows/macOS/Linux, and a systemd user service for a headless Pi
- [Letting an agent drive a browser](https://kittipitch.github.io/smalltv-mod/reference/agent-browsers/) — hand the clicking
  to Claude Code or Codex (BrowserOS neo, ego lite, or an MCP server driving Chrome on any desktop OS). No Docker
- [Building from source](https://kittipitch.github.io/smalltv-mod/reference/building/), and [recovery](https://kittipitch.github.io/smalltv-mod/reference/recovery/)
