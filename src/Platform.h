...
// Crash log: ESP8266-only (RTC user memory + the core's postmortem hook).
// The ESP32 cores expose the panic reason but not epc/excvaddr the same way,
// and no unit in this fleet is an ESP32, so these are deliberate no-ops rather
// than a second half-tested implementation.
static inline bool platformCrashLogRead(String&) { return false; }
static inline void platformCrashLogClear() {}
static inline void platformHealthTick() {}
...
// Crash log persisted in RTC memory by custom_crash_callback() (Platform.cpp).
// Returns false when the last reset was not an exception, or after a power
// cycle -- RTC memory does not survive loss of power, so read this BEFORE
// power-cycling a crashed unit.
bool platformCrashLogRead(String& out);
void platformCrashLogClear();
// Every 30 s from loop(): heap, largest block, Wi-Fi state, RSSI and link-drop
// count into a small RTC ring, so the next crash record can show the run-up to
// the fault -- which the crash handler itself cannot measure (a heap walk there
// can fault on a corrupt heap). ESP8266 only.
...
//    (github.com, raw.githubusercontent.com) then skip ECDHE entirely, keeping
//    those handshakes as light as the old BASIC build. Nothing currently needs
//    the full (heavy) suite list -- the one host that did, the ticker's
//    ECDHE-only cash.ch CDN, went away with that feature.
static inline SecureClient* platformMakeSecureClient(uint16_t rxBuf,
                                                     TlsSession* session = nullptr,
                                                     uint16_t txBuf = 512,
                                                     bool cheapCiphers = false) {
