...
static void drawCodexMeter(Arduino_GFX* gfx, int top, const char* label,
  ...
  // Codex also needs the 2-digit width: it frees one size5 char cell for
  // drawCodexResetSuffix() under the 7d label. (Until 2026-09-12 this page
  // printed nothing at 100% instead.) All branches stay 3 chars wide so the
  // opaque print self-clears on data-only redraws.
  char pc[8];
  int pv = constrain(pct, 0, 100);
  if (has) snprintf(pc, sizeof(pc), "%2d%%", min(pv, 99));
  else     strlcpy(pc, " --", sizeof(pc));
  uint8_t sz = gfxFitSize(pc, 150, 5);
  int pcw = gfxTextW(pc, sz);
  gfx->setTextSize(sz);
  gfx->setTextColor((has && pv >= 100) ? C_RED : C_WHITE, C_PANEL);
...
// own comment on the 2-digit cap) has its cursor at
