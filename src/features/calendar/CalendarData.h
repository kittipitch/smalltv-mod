...
struct WeatherData {
  ...
  uint32_t lastOkMs;
  // Bumped on EVERY mutation, including retirement. lastOkMs only moves on a
  // SUCCESS, so a cycle that retired stale halves changed the data without
  // changing the renderers' dirty signal -- and the cached pixels kept showing
  // readings that had just been thrown away.
  uint32_t renderGen;
