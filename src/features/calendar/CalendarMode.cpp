...
// AQI + PM2.5.
// Local copy of CalendarClient.cpp's filter (that one is file-static). The
// renderer is a CP437 path: a UTF-8 label from the browser's reverse-geocode
// ("Munchen", Thai localities) draws as garbage glyphs, and truncating by bytes
// can split a multi-byte sequence.
static void wxAsciiOnly(const char* in, char* out, size_t outCap) {
  size_t o = 0;
...
static void drawWeatherPage(Arduino_GFX* gfx, const WeatherData& w, const Settings& s) {
  gfx->fillScreen(C_BLACK);
  ...
  // Precedence, most specific first. `place` is the user's own label, set in
  // the web UI (and auto-filled by the browser's reverse-geocode), so it is the
  // only source a daemonless unit can have -- without it such a unit showed the
  // literal "WEATHER" forever. Coordinates beat that generic word: they at least
  // say WHERE the numbers are from.
  {
    char hdr[20];
    if (s.calendar.place.length()) {
      // Sanitize like the pushed city does. `place` can be auto-filled by the
      // browser's reverse-geocode, so "Munchen" or a Thai locality arrives as
      // UTF-8 -- and this renderer is a CP437 path, where those bytes become
      // garbage glyphs and a byte truncation can split a sequence mid-character.
      char clean[24];
      wxAsciiOnly(s.calendar.place.c_str(), clean, sizeof(clean));
      truncateDots(clean, hdr, sizeof(hdr), 18);
    } else if (w.hasCity) {
      truncateDots(w.city, hdr, sizeof(hdr), 18);
    } else if (s.calendar.lat != 0.0f || s.calendar.lon != 0.0f) {
      snprintf(hdr, sizeof(hdr), "%.2f%c %.2f%c",
               fabsf(s.calendar.lat), s.calendar.lat < 0 ? 'S' : 'N',
               fabsf(s.calendar.lon), s.calendar.lon < 0 ? 'W' : 'E');
    } else {
      strlcpy(hdr, "WEATHER", sizeof(hdr));
    }
    drawRow(gfx, 10, 2, C_DIM, hdr);
  }

  // Location never set: say so. The web UI claims this screen prompts for it
  // (webui.h), but nothing here ever tested the coordinates -- a new owner just
  // saw empty placeholders and had no way to know what was missing.
  // Only when there is genuinely nothing to show. A daemon-fed unit can have
  // lat/lon left at 0,0 and still receive perfectly good pushed readings --
  // gating on coordinates alone replaced live data with a setup prompt.
  if (s.calendar.lat == 0.0f && s.calendar.lon == 0.0f &&
      // Every has* flag, not a subset: weatherApply() accepts a push carrying
      // only uvIndex or only aqiNow, and omitting those two let the setup prompt
      // replace a live reading the device actually had.
      !w.hasTemp && !w.hasPrecip && !w.hasWeatherCode && !w.hasAqi && !w.hasPm25 &&
      !w.hasUvIndex && !w.hasAqiNow && !w.hasForecast) {
    gfxDrawCentered("SET LOCATION", 100, 2, C_YELLOW);
    gfxDrawCentered("web UI -> Calendar", 130, 1, C_DIM);
    return;
  }
...
void CalendarWeatherMode::service(const Settings& s) {
  ...
  if (w.renderGen != wxRenderedOk_) { wxRenderedOk_ = w.renderGen; needRender_ = true; }
  if (needRender_) {
    Arduino_GFX* gfx = gfxDev();
    if (gfx) drawWeatherPage(gfx, w, s);
    needRender_ = false;
  }
}
