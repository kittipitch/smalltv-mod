...
// Link health, for /api/status and the outbound-fetch admission checks.
uint32_t netDisconnectCount();  // link drops seen since boot (polled, not event-driven)
uint32_t netDownMs();           // length of the current outage, 0 while connected
uint32_t netUpMs();             // how long the link has been continuously up, 0 while down
