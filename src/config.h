...
#define FW_VERSION       "1.0.0-kitt26"
...
#define MODE_ALBUM      14  // photo page: raw RGB565 frames pushed by the daemon (no decoder on-device)

// ---- album ----------------------------------------------------------------
// Frame geometry is the panel's, and the daemon must match it exactly: any body
// that is not W*H*2 bytes is rejected rather than part-drawn.
#define ALBUM_W 240
#define ALBUM_H 240
#define ALBUM_PATH     "/album.565"
#define ALBUM_TMP_PATH "/album.tmp"
// Write the offline fallback every Nth pushed frame, not every frame: 115,200 B
// per rotation is ~660 whole-filesystem rewrites a day (~5 months of flash);
// every 10th is ~17 years. Costs a repeated picture while the daemon is down.
#define ALBUM_CHECKPOINT_EVERY 10
...
//
//   Weather/AQI has TWO first-class sources. A daemon push (POST /api/weather)
//   wins while one keeps arriving; WX_PUSH_GRACE_MS after the last push the
//   device fetches Open-Meteo itself (WITH_DEVICE_WEATHER, default on).
//
//   NOT a fallback for an edge case -- most owners run the daemon on a laptop or
//   desktop that sleeps, and weather that freezes whenever that machine is off
//   is the difference between a useful unit and an expensive clock. Usage and
//   calendar cannot work this way (Claude credentials, Google OAuth), so they
//   keep displaying their last received values instead. Plain HTTP, so the fetch
//   costs no BearSSL heap.
// ---------------------------------------------------------------------------
...
// a live daemon.
// Set by scripts/git_sha.py at build time. Defined here too so a build that
// bypasses the extra_script (an IDE one-off, a stripped copy of the tree) still
// compiles -- it just cannot identify itself.
#ifndef GIT_SHA
#define GIT_SHA "unknown"
#endif

// Grace period after the last accepted /api/weather push before the device
// starts fetching for itself. See weatherService(); fixed, not a multiple of
// the poll period.
#ifndef WX_PUSH_GRACE_MS
#define WX_PUSH_GRACE_MS 180000UL
#endif

// Hard wall-clock ceiling on ONE weather/AQ request, body included.
// HTTPClient::setTimeout() bounds DNS, connect, header-wait and per-byte body
// gaps SEPARATELY, so a peer that sends one byte before each idle timeout has
// no total bound at all -- and loop() cannot reach webPortalLoop() meanwhile,
// which would make the only OTA recovery path unreachable indefinitely.
#ifndef WX_REQ_DEADLINE_MS
#define WX_REQ_DEADLINE_MS 8000UL
#endif

// Ceiling on the self-fetch backoff after repeated failed cycles.
#ifndef WX_BACKOFF_MAX_MS
#define WX_BACKOFF_MAX_MS 1800000UL
#endif
