...
<main>
 ...
 <section id="display" class="tab">
  <div class="card"><h2>Mode</h2>
   ...
   <select id="mode" onchange="modeChanged()">
    ...
    <option value="album">Pictures</option>
 ...
 <section id="calendar" class="tab">
  ...
  <div class="card"><h2>Weather location</h2>
   ...
   <div class="row">
    <div><label>Place label</label><input id="calPlace" type="text" maxlength="24" placeholder="Chiang Mai"></div>
    <div><label>Refresh every (s)</label><input id="calPoll" type="number" min="60" max="65535" step="60" placeholder="600"></div>
   </div>
   <small class="hint">Decimal degrees, e.g. <code>52.3676</code>, <code>4.9041</code>. Leave at 0/0 and the screen shows <b>SET LOCATION</b> instead of data. <b>Place label</b> is the heading shown on the weather page &mdash; it is filled in for you when you set a location, and it is the only name a device with no daemon can display. <b>Refresh</b> applies to the device's own fetching. Weather/AQI is fetched by the daemon while one is pushing; when pushes stop for 3 minutes this device fetches <a href="https://open-meteo.com" target="_blank">Open-Meteo</a> itself, so the screen keeps updating while your computer is off.</small>
  </div>
 </section>
...
function calUseMyLocation(){
 ...
 fetch('https://ipwho.is/').then(function(r){return r.json()}).then(function(d){
  ...
  // Fill the device-side label from THIS lookup. Without it the one-click flow
  // saved an empty place, and the unit fell through to raw coordinates -- the
  // exact thing the label exists to avoid, in the flow most people use.
  var pl=$('calPlace'); if(pl){delete pl.dataset.userEdited; if(d.city)pl.value=d.city;}
...
function calRefreshLocLabel(lat,lon){
 ...
 fetch('https://api.bigdatacloud.net/data/reverse-geocode-client?latitude='+lat+'&longitude='+lon+'&localityLanguage=en')
  .then(function(r){return r.json()}).then(function(d){
   ...
   // Fill the device-side label from the SAME lookup, so a unit that will never
   // have a daemon still gets a real place name. Only when empty -- never
   // overwrite what the user typed. `locality` beats `city`: BigDataCloud's
   // `city` is often the containing district ("Amphoe Mueang Chiang Mai").
   var pl=$('calPlace');
   // Overwrite an AUTO-derived label, keep a hand-typed one. Refusing to
   // overwrite anything non-empty meant that once a label was derived, moving
   // the coordinates left the old place name captioning the new location's
   // readings -- permanently, since `place` outranks both city and coordinates.
   if(pl&&(!pl.value||!pl.dataset.userEdited)){var shortName=d.locality||d.city||''; if(shortName)pl.value=shortName;}
...
var CAR_MODES=[
 ...
 {id:'album',chk:'carouselAlbum',label:'Pictures'},
...
function loadConfig(){return j('/api/config').then(function(c){C=c;
 ...
 sc('carouselAgenda',c.carouselAgenda!==false); sc('carouselAgenda2',c.carouselAgenda2!==false); sc('carouselWeather',c.carouselWeather!==false); sc('carouselForecast',c.carouselForecast!==false); sc('carouselZai',c.carouselZai!==false); sc('carouselCodex',c.carouselCodex!==false); sc('carouselAntigravity',c.carouselAntigravity!==false); sc('carouselOpenrouter',c.carouselOpenrouter!==false); sc('carouselAlbum',c.carouselAlbum===true);
 // usage slice
 ...
 // until the coordinates actually move.
 if($('calPlace')&&(cal.place||''))$('calPlace').dataset.userEdited='1';
 ...
 var _pl=$('calPlace');
 if(_pl)_pl.oninput=function(){this.dataset.userEdited='1'};
 $('calLat').onchange=$('calLon').onchange=function(){
  var la=parseFloat(gv('calLat'))||0, lo=parseFloat(gv('calLon'))||0;
  resolveTz(la,lo);
  // Release the label so it can be re-derived. The comment on load says a
  // stored label is user-owned "until the coordinates move" -- this is the
  // until; without it a stored name outranked city and coordinates forever and
  // permanently captioned the new location with the old place.
  var pl=$('calPlace'); if(pl)delete pl.dataset.userEdited;
  calRefreshLocLabel(la,lo);
 };
...
function collect(){
 var o={mode:gv('mode'),
  ...
  carouselAgenda:gc('carouselAgenda'), carouselAgenda2:gc('carouselAgenda2'), carouselWeather:gc('carouselWeather'), carouselForecast:gc('carouselForecast'), carouselZai:gc('carouselZai'), carouselCodex:gc('carouselCodex'), carouselAntigravity:gc('carouselAntigravity'), carouselOpenrouter:gc('carouselOpenrouter'), carouselAlbum:gc('carouselAlbum'),
