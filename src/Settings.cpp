...
void CalendarSettings::fromJson(JsonObjectConst o) {
  ...
  if (o["place"].is<const char*>()) {
    // The browser enforces maxlength, but /api/config is a public endpoint on
    // the LAN and nothing else bounded this. The renderer truncates to 18
    // glyphs; 48 bytes is generous for that and keeps a stray payload from
    // parking an arbitrary String in a ~27 KB heap.
    place = o["place"].as<String>();
    if (place.length() > 48) place.remove(48);
  }
  // Bound BOTH ends. Only a lower clamp existed, so 65536 truncated to 0 in the
  // uint16_t and the fetch fell to its 60 s emergency floor while the web UI
  // still displayed the value the user thought they had set.
  if (o["weatherPollSec"].is<int>()) {
    long v = (long)o["weatherPollSec"];
    if (v < 60) v = 60;
    if (v > 65535) v = 65535;
    weatherPollSec = (uint16_t)v;
  }
...
void Settings::setDefaults() {
  ...
  // Off by default: an empty album page is a black screen in the rotation, and
  // nothing shows until the owner points the daemon at a folder.
  carouselAlbum = false;
  carouselOrder = "";
...
void settingsToJson(const Settings& s, JsonObject root, bool includeSecrets) {
  ...
  root["carouselAlbum"]     = s.carouselAlbum;
...
void settingsApplyJson(Settings& s, JsonObjectConst root) {
  ...
  // Daemon IP filter: apply as-is when present (empty allowed => accept a
  // push from anywhere, the default).
  if (root["daemonIp"].is<const char*>()) s.daemonIp = root["daemonIp"].as<String>();
  ...
  if (root["mode"].is<const char*>()) {
    ...
    s.mode = m.equalsIgnoreCase("radar")    ? MODE_RADAR
           ...
           : m.equalsIgnoreCase("album")      ? MODE_ALBUM
           : MODE_CAROUSEL;
  }
  ...
  if (root["carouselAlbum"].is<bool>())   s.carouselAlbum = root["carouselAlbum"];
