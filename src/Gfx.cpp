...
int gfxTextW(const char* s, uint8_t size) { return s ? (int)strlen(s) * 6 * size : 0; }

void gfxDrawCentered(const char* s, int y, uint8_t size, uint16_t color) {
  // NULL is tolerated on purpose: every caller here is a diagnostic path, and
  // several pass a value that can legitimately be absent (an optional reason
  // line, an IP before the interface is up). Print::print(nullptr) would run
  // strlen on 0 and take the whole panel down for a missing label.
  if (!gfx || !s) return;
...
void gfxApInfo(const char* ssid, const char* pass, const char* ip, const char* mac) {
  if (!gfx) return;
  gfx->fillScreen(C_BLACK);
