...
- **The image is one build, not two.** `WITH_RADAR=0` (every
  target compiles these out now, not just this board) brings it to ~710 KB
  (708,224 B in the current release), fitting the loader's own OTA slot with
  room to spare. There is one build per target with TLS in, so GitHub
  self-update works normally. "~710 KB" sized at release time; builds move a
  few KB as features land.
