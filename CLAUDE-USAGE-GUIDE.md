...
- **This firmware fork ([`kittipitch/smalltv-mod`](https://github.com/kittipitch/smalltv-mod))**
  ...
  Agenda + Weather/AQI mode, plane radar, an optional daemon source-IP
  filter, display color correction, user-configurable carousel order, and a
...
pin the display to one mode. Every quota page (Claude, z.ai, Codex,
Antigravity) prints at most two digits: a quota that is fully used shows
**99% in red** over a full red bar, so a red 99% always means 100%.
...
- **z.ai** — 5h cycle % and a monthly MCP-tools quota %, each with a reset
  countdown
- **Codex** — 5h and 7d quota % + reset countdowns, plus a small line inside
  the 7d card with free "rate-limit reset" credits and when the soonest one
  expires unused
...
(`--push-to smalltv-3fa2.local`, or whatever hostname your device shows in
...
3. Run it pointed at your device:
   ```
   .venv/bin/python clawdmeter_daemon.py --push --push-to <device-ip-or-hostname> --no-discover --interval 300
   ```
...
**Do this before you first start the daemon.** Starting it without a token just polls
forever with no `5h=` line in the log.

**Getting a Claude token — never paste a token into chat:**
If you already use `claude` on the machine that will run the daemon, there is nothing to
do: the daemon reads that login (macOS Keychain, or `~/.claude/.credentials.json`).

For a machine that never runs `claude` interactively — or when the tray says *Token
expired* — run `claude setup-token` there. It **prints** a long-lived token starting
`sk-ant-oat…`; it does not store it for you. Put it in a `.env` file beside the script as
`CLAUDE_CODE_OAUTH_TOKEN=sk-ant-oat...`, or set it as an environment variable. Type or
paste it straight into that file — never into a chat window. A paid Claude plan is
required for `setup-token`.
...
1. `.venv/bin/python clawdmeter_daemon.py --calendar-auth` — one-time interactive
...
**Optional: daemon source IP**
If you want to stop a daemon accidentally pointed at the wrong device from
overwriting the screen, set a "Daemon source IP" in the device's web UI
(Update tab). Nothing to configure on the daemon side — it's a device-side
filter only, and it's not a security feature (this is plaintext HTTP on your
own LAN). Leave it unset if you don't need it (the device accepts pushes
